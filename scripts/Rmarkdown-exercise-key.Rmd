...
library(tidyverse)
library(here)
df = read.csv(here("data", "sound-production-treatment.csv"), stringsAsFactors = TRUE)
```
...
```{r}
# filter for all resposne values that are not NA
df_noNA <- df %>% 
...
# This way drops too many rows!! Can you tell why?
df_noNA_bad <- df %>%
  drop_na()
...
# Your code here
df_summary <- df_noNA %>%
  # filter for each of the aspects of the data we want. | is for or
  filter(phase == "baseline" | phase == "treatment" | phase == "followup",
         condition == "blocked",
         itemType == "tx") %>%
  group_by(participant, session) %>%
  summarize(mean_accuracy = mean(response))
...
df_summary <- df_noNA %>%
  ...
  filter(phase == "baseline" | phase == "treatment" | phase == "followup",
         condition == "blocked",
         itemType == "tx") %>%
  # Including phase in the group_by doesn't change the grouping because session
  # is more detailed than phase, (within participant, sessions are in the same phase)
  # and it keeps phase in the dataset
  group_by(participant, session, phase) %>%
  summarize(mean_accuracy = mean(response))
...
Participant 10's baseline accuracy was `r P10_bl_all`%.
...
-   Bonus: Change the colors used in the plot

First, create the data we want to use:

```{r}
# Your code here
df_p5 <- df_noNA %>%
  filter(participant == "P5") %>%
  group_by(phase, session, itemType, condition) %>%
  summarize(mean_accuracy = mean(response)) 
```
...
```{r}
df_p5 %>%
  ggplot(aes(x = session, y = mean_accuracy, shape = phase, color = itemType)) +
  geom_point() + 
  geom_line() + 
  facet_wrap(~condition, ncol = 1) +
  labs(y = "Percent Correct",
       title = "Participant 5's performance over time") +
  theme_classic() + 
